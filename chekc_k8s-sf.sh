...
check_etcd() {
  ...
  if ! ss -ntlp|grep etcd > /dev/null; then
    ...
    if [[ "$serverStatus" == "active" ]];then
      ...
      red  "  └──[Error] kube-controll health check isn't active。$serverStatus"
    fi
...
get_cluster_resources() {
  ...
  while IFS= read -r line;do
    ...
    if [[ "$nodeRequestsCpuPercentNum" -gt 80 ]];then
      yellow "  └──[Warning] node($nodeName) request cpu used grater than 80%，request：$nodeRequestsCpu；max：${nodeCapacityCpu}m；percent：$(echo "$nodeRequestsCpuPercent"|sed 's/(//;s/)//')"
    else
    ...
    if [[ "$nodeRequestsMemPercentNum" -gt 80 ]];then
      yellow "  └──[Warning] node($nodeName) request mem used grater than 80%，request：$nodeRequestsMem；max：${nodeCapacityMem}Mi；percent：$(echo "$nodeRequestsMemPercent"|sed 's/(//;s/)//')"
    else
...
check_resources_request() {
  ...
  if [[ "$clusterRequestsMem" -gt $((clusterCapacityMem * 8 / 10)) ]];then
    yellow "  └──[Warning] cluster request mem used grater than 80%，used：${clusterRequestsMem}Mi，max: ${clusterCapacityMem}Mi，$(awk 'BEGIN{printf "%.0f\n","'"${memUsedPercent}"'"*100}')%"
  else
  ...
  if [[ "$clusterRequestsCpu" -gt $((clusterCapacityCpu * 8 / 10)) ]];then
    yellow "  └──[Warning] cluster request cpu used grater than 80%，used：${clusterRequestsCpu}m，max: ${clusterCapacityCpu}m，$(awk 'BEGIN{printf "%.0f\n","'"${cpuUsedPercent}"'"*100}')%"
  else
